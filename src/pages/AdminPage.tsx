...
import {
  ...
  ArrowUp, ArrowDown, LogIn, LogOut, Lock, User, Pencil,
} from 'lucide-react';
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-2xl p-8">
          ...
          <div className="mt-6 text-center">
            <Link to="/" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" />返回前台</Link>
          </div>
        </div>
      </div>
    </div>
  ...
  const handlePersonSelect = (personId: string) => {
    ...
    if (person) {
      const fieldValues: Record<string, string> = {
        ...
        biography: person.biography, spouse: person.spouse || '',
        achievements: person.achievements?.join('\n') || '',
      };
      setEditForm(prev => ({ ...prev, personId, oldValue: fieldValues[prev.field] || '' }));
    }
  };

  const filteredFeedbacks = useMemo(() => feedbacks.filter(f => {
    const matchSearch = !searchTerm || f.personName.toLowerCase().includes(searchTerm.toLowerCase()) || f.genealogyName.toLowerCase().includes(searchTerm.toLowerCase()) || f.description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchStatus = statusFilter === 'all' || f.status === statusFilter;
    return matchSearch && matchStatus;
  }), [feedbacks, searchTerm, statusFilter]);

  const filteredEdits = useMemo(() => edits.filter(e => {
    const matchSearch = !searchTerm || e.personName.toLowerCase().includes(searchTerm.toLowerCase()) || e.genealogyName.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-card border border-border rounded-xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm"><span className="text-muted-foreground">已驳回</span><span className="font-medium text-red-600">{stats.feedbacks.rejected}</span></div>
                </div>
              </div>
              <div className="bg-card border border-border rounded-xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm"><span className="text-muted-foreground">已驳回</span><span className="font-medium text-red-600">{stats.edits.rejected}</span></div>
                </div>
              </div>
              <div className="bg-card border border-border rounded-xl p-6">
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-border"><h3 className="font-semibold text-foreground">最近反馈</h3></div>
              {feedbacks.length === 0 ? (<div className="p-8 text-center text-muted-foreground"><p className="text-sm">暂无反馈记录</p></div>) : (
                <div className="divide-y divide-border">
                  {feedbacks.slice(0, 5).map(fb => (
                    <div key={fb.id} className="px-6 py-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${fb.status === 'pending' ? 'bg-amber-500' : fb.status === 'resolved' ? 'bg-green-500' : 'bg-red-500'}`} />
                        <div><div className="text-sm font-medium text-foreground">{fb.genealogyName} - {fb.personName}</div><div className="text-xs text-muted-foreground">{feedbackTypeLabels[fb.feedbackType]} · {new Date(fb.createdAt).toLocaleDateString('zh-CN')}</div></div>
                      </div>
                      <span className={`text-xs px-2 py-1 rounded-full ${fb.status === 'pending' ? 'bg-amber-100 text-amber-700' : fb.status === 'resolved' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{fb.status === 'pending' ? '待处理' : fb.status === 'resolved' ? '已解决' : '已驳回'}</span>
                    </div>
                  ))}
                </div>
              )}
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mb-1"><span className="text-sm font-semibold text-foreground">{fb.genealogyName}</span><span className="text-muted-foreground">→</span><span className="text-sm font-medium text-primary">{fb.personName}</span></div>
        ...
        {activeTab === 'edits' && (
          <div className="space-y-6 animate-fade-in">
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => setShowEditForm(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm hover:bg-primary/90 transition-colors"><Edit3 className="w-4 h-4" />新增修改</button>
            </div>
            ...
            {showEditForm && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                <div className="bg-card border border-border rounded-2xl max-w-lg w-full animate-scale-in overflow-hidden">
                  <div className="px-6 py-4 border-b border-border flex items-center justify-between"><h3 className="text-lg font-semibold text-foreground">新增人物修改</h3><button onClick={() => setShowEditForm(false)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-secondary transition-colors"><XCircle className="w-4 h-4 text-muted-foreground" /></button></div>
                  <form onSubmit={handleSubmitEdit} className="p-6 space-y-4">
                    <div><label className="block text-sm font-medium text-foreground mb-1">选择族谱</label><select value={editForm.genealogyId} onChange={(e) => setEditForm(prev => ({ ...prev, genealogyId: e.target.value, personId: '', oldValue: '' }))} className="w-full px-4 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required><option value="">请选择族谱</option>{genealogies.map(g => (<option key={g.id} value={g.id}>{g.name}</option>))}</select></div>
                    {editForm.genealogyId && (<>
                      <div><label className="block text-sm font-medium text-foreground mb-1">选择人物</label><select value={editForm.personId} onChange={(e) => handlePersonSelect(e.target.value)} className="w-full px-4 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required><option value="">请选择人物</option>{availablePersons.sort((a, b) => a.generation - b.generation).map(p => (<option key={p.id} value={p.id}>第{p.generation}世 - {p.name}</option>))}</select></div>
                      {editForm.personId && (<>
                        <div><label className="block text-sm font-medium text-foreground mb-1">修改字段</label><select value={editForm.field} onChange={(e) => { const field = e.target.value; const genealogy = getGenealogy(editForm.genealogyId); const person = genealogy?.people[editForm.personId]; const fv: Record<string, string> = { name: person?.name || '', birthYear: person?.birthYear || '', deathYear: person?.deathYear || '', biography: person?.biography || '', spouse: person?.spouse || '', achievements: person?.achievements?.join('\n') || '' }; setEditForm(prev => ({ ...prev, field, oldValue: fv[field] || '' })); }} className="w-full px-4 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary">{Object.entries(fieldLabels).map(([key, label]) => (<option key={key} value={key}>{label}</option>))}</select></div>
                    ...
                    <div className="flex gap-3 pt-2"><button type="button" onClick={() => setShowEditForm(false)} className="flex-1 px-4 py-2.5 bg-secondary text-secondary-foreground rounded-lg hover:bg-secondary/80 transition-colors">取消</button><button type="submit" className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors"><Save className="w-4 h-4" />提交修改</button></div>
                  </form>
                </div>
              </div>
            )}
                  ...
                  <div key={edit.id} className="bg-card border border-border rounded-xl overflow-hidden">
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          {edit.status === 'pending' && (<><button onClick={() => handleEditAction(edit.id, 'approved')} className="inline-flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs hover:bg-green-700 transition-colors"><CheckCircle2 className="w-3.5 h-3.5" />通过</button><button onClick={() => handleEditAction(edit.id, 'rejected')} className="inline-flex items-center gap-1 px-3 py-1.5 bg-destructive/10 text-destructive rounded-lg text-xs hover:bg-destructive/20 transition-colors"><XCircle className="w-3.5 h-3.5" />驳回</button></>)}
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                <div className="bg-card border border-border rounded-2xl max-w-lg w-full animate-scale-in overflow-hidden max-h-[90vh] overflow-y-auto">
                  ...
                  <form onSubmit={handleSubmitNewPerson} className="p-6 space-y-4">
                    <div><label className="block text-sm font-medium text-foreground mb-1">选择族谱 <span className="text-destructive">*</span></label><select value={newPersonForm.genealogyId} onChange={(e) => setNewPersonForm(prev => ({ ...prev, genealogyId: e.target.value }))} className="w-full px-4 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required><option value="">请选择族谱</option>{genealogies.map(g => (<option key={g.id} value={g.id}>{g.name}</option>))}</select>{formErrors.genealogyId && <p className="text-xs text-destructive mt-1">{formErrors.genealogyId}</p>}</div>
                    {newPersonForm.genealogyId && (<>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div><label className="block text-sm font-medium text-foreground mb-1">逝世年份</label><input type="text" value={newPersonForm.deathYear} onChange={(e) => setNewPersonForm(prev => ({ ...prev, deathYear: e.target.value }))} placeholder="如：1980" className="w-full px-4 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" /></div>
                      </div>
                      <div><label className="block text-sm font-medium text-foreground mb-1">父亲（选填）</label><select value={newPersonForm.parentId} onChange={(e) => setNewPersonForm(prev => ({ ...prev, parentId: e.target.value }))} className="w-full px-4 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"><option value="">无（新增分支始祖）</option>{availableParents.map(p => (<option key={p.id} value={p.id}>第{p.generation}世 - {p.name}</option>))}</select>{newPersonForm.parentId && (<p className="text-xs text-muted-foreground mt-1">提示：世系将自动设为父亲世系 + 1（第{(() => { const p = selectedGenealogy?.people[newPersonForm.parentId]; return p?.generation ? p.generation + 1 : '?'; })()}世）</p>)}</div>
                  ...
                  <div key={person.id} className="bg-card border border-border rounded-xl overflow-hidden">
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${person.status === 'pending' ? 'bg-amber-100 text-amber-700' : person.status === 'approved' ? 'bg-green-100 text-green-700' : person.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>{person.status === 'pending' ? '待审核' : person.status === 'approved' ? '已通过' : person.status === 'rejected' ? '已驳回' : person.status}</span>
                          ...
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div><div className="text-lg font-semibold text-foreground">{person.name}</div><div className="text-xs text-muted-foreground">{person.genealogyId && genealogies.find(g => g.id === person.genealogyId)?.name}{person.birthYear && ` · ${person.birthYear}-${person.deathYear || '？'}`}</div></div>
                          </div>
                          {person.biography && <p className="text-sm text-foreground mt-2 line-clamp-2">{person.biography}</p>}
                          {person.parentId && (() => { const g = getGenealogy(person.genealogyId); const parent = g?.people[person.parentId]; return parent ? <p className="text-xs text-muted-foreground mt-1">父亲：{parent.name}</p> : null; })()}
          ...
          <div className="bg-card border border-border rounded-2xl max-w-lg w-full animate-scale-in overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2 mb-3"><span className="text-sm font-semibold text-foreground">{selectedFeedback.genealogyName}</span><span className="text-muted-foreground">→</span><span className="text-sm font-medium text-primary">{selectedFeedback.personName}</span></div>
